        ...
        for index, job in enumerate(jobs, start=1):
            ...
            lines.extend(
                [
                    ...
                    f"- Vehicle support: {'yes' if job.get('vehicle_support_signal') else 'no'}",
                    f"- Vehicle barrier: {'yes' if job.get('vehicle_barrier') else 'no'}",
                    f"- License required: {'yes' if job.get('license_required_signal') else 'no'}",
                    f"- Driving record: {'clean' if job.get('driving_record_signal') else 'unknown'}",
                    f"- Travel supported: {'yes' if job.get('travel_supported_signal') else 'no'}",
